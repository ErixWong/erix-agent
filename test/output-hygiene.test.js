...
test("oversized tool result: context gets a stub and the record keeps full content", async () => {
  ...
  assert.match(String(stubResult?.content), /先用 note_list 查找记录，再用 note_read 读取/);
  ...
  // 3. host only executes the declared tool; archived bytes remain in the record.
...
test("archive stub warns about side effects of the truncated command", async () => {
  ...
  assert.match(stub, /先用 note_list 查找记录，再用 note_read 读取/);
  // issue #32：补副作用提示——不得仅凭截断输出判成败，也不得为重跑补全而重复副作用命令
  assert.match(stub, /若原命令有副作用，不要仅凭截断输出判断成败/);
  assert.match(stub, /也不要为补全输出重跑有副作用的命令/);
  assert.match(stub, /若未记录且无法确定性重算，请省略对应 findings 声明/);
});

test("crash mid-round: archived output survives via checkpoint", async () => {
  const store = createMemoryTranscriptStore();

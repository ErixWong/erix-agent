...
This document was first drafted on 2026-08-29 and has been reconciled with the implementation shipped in version 0.8.0. It describes the library's actual scope and APIs, not an unqualified list of future plans.
...
3. Provide pluggable context-budget compaction: `sliding-window`, `fold-statistical`, and `fold-llm`, with full folded payloads retained in checkpoints while request views can use TTL folding.
...
- ❌ Do not ship the planned `psyche` compaction strategy in the 0.8.0 runtime. Its context-shaping idea remains a future, conversation-oriented design candidate rather than a current implementation.
...
| # | Requirement | Status and implementation |
|---|---|---|
| FR-2.1 | The host injects `tools` with standard JSON Schema definitions and a structured `executeTool({ id, name, input, context, signal })` callback; the library does not own the execution policy | **Delivered.** `runToolLoop` passes the schemas to the provider and invokes the callback with exactly one structured object. |
| FR-2.2 | Retry a retryable provider failure or a present-but-empty assistant response from an in-round snapshot, with a default of two retries and exponential backoff from 1.5s to a 10s cap; throw after the retries are exhausted | **Delivered.** With `retry: {}`, `runToolLoop` defaults to two retries, a 1.5s base delay, and a 10s cap, restores the round snapshot, and retries only retryable failures. A present-but-empty assistant response is marked retryable; `ERIX_RETRY_ATTEMPTS` overrides the CLI default. Tool execution itself is not automatically retried. |
...
| FR-3.6 | Track folded rounds and preserve their full payload in `TranscriptStore`; keep request-view folding separate from checkpoint data | **Delivered.** `runToolLoop` tracks folded ranges and records `foldedPayload` in persisted round/checkpoint data. Tool-result TTL folding is applied only to the request view: old, large results become navigation-digest/JSON-skeleton placeholders, while checkpoints retain the full text. The warning round gives the model a chance to extract notes before folding. |
...
| FR-5.2 | Provide an optional `erix-agent/tools` subpath with tool registration and tool providers | **Delivered.** `package.json` exports `./tools` to `src/tools/index.js`, which exports the tool registry and tool providers. These are opt-in and are not automatically installed as loop tools; no path-jail or filesystem helper is included. |
...
The original phase plan is reconciled below with what exists in package version 0.8.0. “Delivered” describes repository code; consumer migrations and real-provider benchmark runs remain external acceptance work.
...
| **v0.2 — delivered** | JSONL file store, JSON-file config, `fold-llm` with an injected summarizer, and the optional `erix-agent/tools` export: registry and tool providers | Crash repair/resume and folded-payload persistence are implemented in the stores and loop. The host still chooses whether to use reference tools. |
...
| **v0.5.0–v0.7.x — superseded** | Earlier archive/retrieval experiments, deterministic run state, fold navigation records, and resume-safe state persistence | The release history records these changes; the current public surface is the 0.8.0 note-first workflow and request-view TTL folding. |
| **v0.8.0 — current** | Note-first retrieval (`note_list` → `note_read`), request-view tool-result TTL folding, retryable empty assistant responses, CLI `--tools` capability allowlists, the pure-node `grep` tool, language-aware system-prompt instructions, and efficient judge/final-round behavior | `ERIX_TOOL_RESULT_TTL` defaults to `2` (`0` disables folding), `ERIX_TOOL_RESULT_FOLD_MIN_TOKENS` defaults to `4000`, and `ERIX_RETRY_ATTEMPTS` overrides the CLI retry count. Intercept budget is 6,000 tokens, round-judge output is capped at 1,024 with no reasoning effort, raw judge output is retained, and final budget rounds send no tools. |
| **v1.0 candidate — not shipped** | A touwaka migration, initially limited to token utilities and the history compactor, with the full `AgentLoop` migration left as a separate decision | No touwaka migration is part of this repository's 0.8.0 implementation. Consumer-side regression and behavior checks must be run by the host project. |
| **v2 candidate — deferred** | Cold-loop distillation plus L3 fact injection for the context-shaping idea, and native Gemini support if a future provider-layer reassessment justifies it | No `psyche` or Gemini-native provider is present in `src/` at 0.8.0. This remains separately scoped work. |
...
- Compaction, TTL folding, retry, protocol adaptation, checkpoint/resume, reflection, and note behavior are locked down by `node --test` tests, with provider tests using mock `fetch`. External consumer migrations and real-relay behavior require their own tests.
...
| Touwaka migration regression: its `AgentLoop` contains the R15/R16/R19 production fixes | Keep the touwaka migration outside the 0.8.0 claim; migrate the pure utility layer first and make the full loop a separate decision with host-side regression tests. |
...
| Consumers defer migration because the immediate benefit does not offset integration cost | Treat consumer migration as an explicit host-project milestone. The 0.8.0 library is more than a utility package, but it cannot claim a migration that is not present in this repository. |

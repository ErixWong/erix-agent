...
> **档案** = `TranscriptStore` 持久化的 round 记录（转录即档案，ADR-015）。
> 另附：**机制解析**（上下文塑形 / 检查点续接 / Judge 治理 / 档案与召回）与**架构评审说明**（文末 Q&A）。
>
...
- **`erix-agent/tools` 是可选子路径**：工具注册表、工具 provider（static/json-file/composite）、recall 工具——宿主可按需取用，不是隐式安装进 runToolLoop 的工具集
...
sequenceDiagram
    ...
    Note over L: recall / outputHygiene 能力检查<br/>（显式 true 但能力缺失 = 抛错，不静默承诺）
    end
    ...
    alt stopReason = tool_use
        ...
        T-->>L: tool_result（大输出归档为 stub，模型只见摘要，ADR-015）
...
- **阶段 2 的工具路径带审计与检查点**：拦截 judge 只拦"写路径"（readFile/tree/rg/note_read/note_list/recall 只读工具豁免）；pre-tool checkpoint 失败直接阻断执行，post-tool 失败标记 `executed_uncommitted` 但结果保留
...
- **折叠产物随档案保存**：`foldedPayload` + 导航记录（fold-statistical 的确定性工具足迹）进入 round 记录——模型忘了细节可用 recall 找回，档案是恢复来源（ADR-015：折叠改变模型视图，不改变档案源）
...
- **只读工具豁免**：readFile / tree / rg / note_read / note_list / recall 被拦净收益为负（实测拦 readFile 反而漏缺陷）——审计对这类调用直接放行，写路径（exec/writeFile/mcp 等）维持拦截语义
...
## 机制解析 4 · 档案与召回（ADR-015：转录即档案，recall 即通道）
...
flowchart TB
    ...
    subgraph surfaces["三条消费通道"]
        REC["引擎 recall 工具<br/>（自动注册，宿主同名优先）"]
        BR2["boundedRecall()<br/>游标分页 + pattern 过滤"]
...
- **召回是有界协议**：`boundedRecall` 的游标绑定 run / 区间 / pattern / limit / artifactRef / **源版本**；源变了报 `stale`、缺记录报 `unrecoverable`、超大记录报 `record_too_large`——不返回"看似成功"的脏数据
- **引擎 recall 工具零配置**：有 store + runId 即自动注册（宿主自带同名 recall 定义时宿主优先）；模型被压缩掉早期上下文后可用 recall 自己找回
...
| 为什么内部要一套规范消息模型，而不是透传 OpenAI/Anthropic？ | 双协议只是当下现实，不是架构中心。规范 Block 层让 compaction/judge/recall 只面向一种结构；新增协议 = 加一对适配器，不动引擎。`validateMessages` 在每次 provider 调用前断言协议不变量。 |
...
| LLM 生成的折叠摘要不可信怎么办？ | 三层防守：① 摘要在进上下文前过确定性 `enforceSize`；② 锚点索引/用户输入引用是机械抽取、不经 LLM；③ 原文保留在 `foldedPayload` 入档案，模型可用 recall 找回。摘要只损失"叙述"，不损失"事实"。 |
...
| compaction 会不会把上下文剪到任务做不完？ | 可能，所以配套：governor 检测"失忆应答"（模型说任务已完成但无工具足迹）并注入恢复提示 + recall 通道找回早期上下文；fold-statistical 留下确定性导航记录；引擎承认压缩有损，把"找回"做成一等能力而不是假装无损。 |

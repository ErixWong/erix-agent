...
- `--no-notes`（或 `ERIX_NO_NOTES=1`）不装配 notes 工厂（`note_*` 工具不再可用），保留其他已加载的 skill。
...
内置的 `notes` 工具提供 `note_take`、`note_read`、`note_list` 和 `note_forget`。它是一个面向 run、pull-only 的便利索引，用于事实、一次性值、决策和 artifact 引用；它不是逐轮日志，也不是 provenance 证据源（guard 的证据是 transcript 的归档输出）。headless 宿主可以从包根或 `erix-agent/tools` 通过 `createBuiltinNotesTools` 全套装配工厂注册；工厂返回 6 键 canonical API（`definitions`、双视图执行器 `executors` / 结构化 `executeTool`、`resolveTools`、run 生命周期钩子 `lifecycle.onRunStart` janitor / `lifecycle.onRunComplete` completeRun 后接 janitor、ADR-015 折叠点 `semanticStateProvider`）——全部绑定同一 run scope 与同一 `NotesStore` 实例。需要 `ToolProvider` 形态的宿主可用 `createStaticToolProvider({ sets: { default: notes.definitions } })` 自建。CLI 已于 v0.11.0 退役 bundled `skills/notes/skill.mjs` 兼容 shim（issue #61）：启用 notes 时统一经工厂装配（可用 `--no-notes` / `ERIX_NO_NOTES=1` 关闭），`erix skills` 不再列出 bundled notes skill；用户和项目 skill 仍可从 `~/.erix/skills/`、项目的 `.erix/skills/` 或 `--skills-dir <path>` 提供。`erix skills` 会列出发现的 skill。

...
- **auto_capture（值 + 引用）**：CLI 在 `exec` 工具执行完成时、而不是折叠时，从完整返回输出中最多捕获 3 个带标签的单行候选；通过凭据过滤且不超过 256 字符的短值同时写入 `content` 与 `{artifactId/archivePath, digest, locator}` 引用，`pinned=true` 和 `provenance.source=auto`，超长值回退为仅引用。模型调用 `note_take` 传入的 provenance 一律按 `source=agent` 记录；即使宿主直接调用导出的 capture helper 写入 `source=auto`，它也只是便利索引，最终核验不信任 notes。归档引用的 digest 始终对应磁盘实际内容；截断归档另记 `truncated=true`、`originalBytes`，不能作为核验依据。短值通过 `note_list → note_read` 一步取回，引用仍保留用于审计与必要的有界核对；同一 key 的后续不同 digest 使用候选 key，不会覆盖首次值，完全相同 digest 去重。非幂等命令即使输出很短也强制写 `<序号>-exec.txt` sidecar 和 `.meta.json`，元数据标明 `replayable=false`；识别包括 `/dev/urandom`、`$RANDOM`、`openssl rand`、`uuidgen`、`date +%s%N`、`mktemp`、`shuf` 及随机字节转 base64 管道。系统会 fail-closed 跳过 `token/key/secret/password/passwd/bearer/authorization/cookie/credential/private/api_key/access_key/refresh_token`、PEM/JWT/AWS/GitHub/npm token、带凭据 URL 和疑似高熵长 base64/hex，**不会把疑似凭据写入 notes**。
...
- 需要恢复具体值时优先按 `note_list → note_read`；只有仅引用型笔记才按返回的 `artifactRef.archivePath` 与 `locator` 有界读取归档并声明核对结果，不得重跑命令或凭记忆补值。归档保存原文，notes 保存短值与审计引用，两者职责不同。
